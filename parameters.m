...
params.D = 7.67;                       % Diameter of the main rotor [m]
params.R = params.D/2;                 % Radius of the main rotor [m]
...
params.xroot = 0.55/11.8;              % X coordinate for the blade root
params.ecc = params.xroot*params.R;    % Blade eccentricity
params.deltaX = 0.0889;                % X deviation of the center of gravity [m]
params.deltaY = 0.026;                 % Y deviation of the center of gravity [m]
params.deltaZ = 1.75/2;                % Z deviation of the center of gravity [m]
params.R_tail_rotor = 1.07/2;          % Tail rotor radius [m]
params.L_tail_rotor = params.R + params.R_tail_rotor; % Tail rotor distance from main rotor [m]  
...
params.cd = @(x) params.delta0 + params.delta1*x + params.delta2.*x.^2; % Drag polar formula
...
params.Iy = params.m_blade*params.R^2 / 4 + (params.m_blade / (params.R * integral(@(x) params.c(x),0,1)))* integral(@(x) params.c(x),0,1)*(params.R^3 / 12); % Blade Y moment of inertia
params.V = 40; % Forward speed [m/s]
